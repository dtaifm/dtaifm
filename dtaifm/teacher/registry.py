...
``base_url``, ``timeout``) through uniformly. Built-in factories use only the
keys they care about and ignore the rest. The mock teacher is always
available; cloud and local adapters import their backing modules lazily so the
core install has no provider dependencies.
"""
...
def register_teacher(name: str, factory: TeacherFactory) -> None:
    ...
    Factories should accept ``**kwargs`` and may consume ``model``,
    ``base_url``, and ``timeout``. They must ignore unknown options
    (typically via ``**_``).
    """
...
def _make_ollama(
    ...
    **_: Any,
    ...
    return OllamaTeacher(model=model, base_url=base_url, timeout=timeout)
...
def _make_lemonade(
    ...
    **_: Any,
    ...
    return LemonadeTeacher(model=model, base_url=base_url, timeout=timeout)
